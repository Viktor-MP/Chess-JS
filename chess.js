...
const chessChart = [
    {
        color: "black",
        name: "rook",
        imgFile: "./BlackFigures/black-rook.png",
        position: ["A8"],
        id: "A8",
    },
    {
        color: "black",
        name: "knight",
        imgFile: "./BlackFigures/black-knight.png",
        position: ["B8"],
        id: "B8",
    },
    {
        color: "black",
        name: "bishop",
        imgFile: "./BlackFigures/black-bishop.png",
        position: ["C8"],
        id: "C8",
    },
    {
        color: "black",
        name: "queen",
        imgFile: "./BlackFigures/black-queen.png",
        position: ["D8"],
        id: "D8",
    },
    {
        color: "black",
        name: "king",
        imgFile: "./BlackFigures/black-king.png",
        position: ["E8"],
        id: "E8",
    },
    {
    ...
    },
    {
        color: "black",
        name: "pawn",
        imgFile: "./BlackFigures/black-pawn.png",
        position: ["A7"],
        id: "A7",
    },
    {
    ...
    },
    {
        color: "white",
        name: "rook",
        imgFile: "./WhiteFigures/white-rook.png",
        position: ["A1"],
        id: "A1",
    },
    {
        color: "white",
        name: "knight",
        imgFile: "./WhiteFigures/white-knight.png",
        position: ["B1"],
        id: "B1",
    },
    {
        color: "white",
        name: "bishop",
        imgFile: "./WhiteFigures/white-bishop.png",
        position: ["C1"],
        id: "C1",
    },
    {
        color: "white",
        name: "queen",
        imgFile: "./WhiteFigures/white-queen.png",
        position: ["D1"],
        id: "D1",
    },
    {
        color: "white",
        name: "king",
        imgFile: "./WhiteFigures/white-king.png",
        position: ["E1"],
        id: "E1",
    },
    {
    ...
    },
    {
        color: "white",
        name: "pawn",
        imgFile: "./WhiteFigures/white-pawn.png",
        position: ["A2"],
        id: "A2",
    },
    {
...
const eateing = (e) => {
    ...
    if (
        child.dataset.type == "pawn" &&
        child.parentElement.dataset.y == 8 &&
        child.dataset.color == "white"
    ) {
        changePawnTo(child.dataset.color, child.parentElement);
    } else if (
        child.dataset.type == "pawn" &&
        child.parentElement.dataset.y == 1 &&
        child.dataset.color == "black"
    ) {
        changePawnTo(child.dataset.color, child.parentElement);
    } else {
        console.log("hello");
...
const changePawnTo = (color, parentBox) => {
    ...
    color === "white"
        ? (toCastlWith.style.top = 0)
        : (toCastlWith.style.bottom = 0);
...
const goingTo = (e) => {
    ...
    data.type == "rook" && e.target.dataset.x == 1 && e.target.dataset.y == 1
        ? (whiteKingLongStep = false)
        : null;
    data.type == "rook" && e.target.dataset.x == 8 && e.target.dataset.y == 1
        ? (whiteKingShortStep = false)
        : null;
    data.type == "rook" && e.target.dataset.x == 1 && e.target.dataset.y == 8
        ? (blackKingLongStep = false)
        : null;
    data.type == "rook" && e.target.dataset.x == 8 && e.target.dataset.y == 8
    ...
    if (
        data.type == "pawn" &&
        data.color == "white" &&
        e.target.dataset.y == 8
    ) {
        changePawnTo(data.color, box);
    } else if (
        data.type == "pawn" &&
        data.color == "black" &&
        e.target.dataset.y == 1
    ) {
        changePawnTo(data.color, box);
    } else {
        removeEvents();
    ...
    board.reverse().map((box) => {
        if (cycle == 1) {
            if (
                ...
                box.children[0].dataset.color != color &&
                box.dataset.x == +x - 1 &&
                box.dataset.y == +y
            ) {
                addEdibleEvent(box);
...
const knight_top_buttom_check = (box, color) => {
    if (
        box.children.length > 0 &&
        box.children[0].dataset.color != color &&
        box.children[0].dataset.type != "king"
    ) {
        addEdibleEvent(box);
        removableBoxes.push(box);
    } else if (
        ...
        board.filter((box) => {
            if (box.dataset.x == 1 && box.dataset.y == y) {
                box.children[0].addEventListener("click", () =>
                    castlToLong(x, y, box.children[0], king)
                );
    ...
    if (charCount == 2) {
        board.filter((box) => {
            if (box.dataset.x == 8 && box.dataset.y == y) {
                box.children[0].addEventListener("click", () =>
                    castlToShort(x, y, box.children[0], king)
                );
                box.classList.add("castling");
                castlingArr.push(box);
            }
...
const castling = (x, y, color, box) => {
    ...
    whiteKingShortStep && color == "white"
        ? shortSideCastling(x, y, box)
        : null;
    blackKingLongStep && color == "black" ? longSideCastling(x, y, box) : null;
    blackKingShortStep && color == "black"
        ? shortSideCastling(x, y, box)
        : null;
};
...
const table = () => {
    numArr.map((line, x) =>
        letterArr.map((calum, y) => {
            let div = document.createElement("div");
            div.className = "chessBord";
            x % 2 === 0
                ? y % 2 === 0 //
                    ? div.classList.add("white")
                    : div.classList.add("black") //  sharum 1 dashti guynery
                : y % 2 === 0
